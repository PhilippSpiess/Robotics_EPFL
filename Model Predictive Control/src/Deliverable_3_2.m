...
i = 1;
try
while i<50 % Simulate until convergence

ux(:,i) = mpc_x.get_u(x(:,i), x_position_reference);
...
uy(:,i) = mpc_y.get_u(y(:,i), y_position_reference); %Take the discretized matrices
...
uz(:,i) = mpc_z.get_u(z(:,i), z_position_reference); %Take the discretized matrices
...
uyaw = [];
yaw0 = [0; 0];
yaw_position_reference = -0.785;
yaw(:,1) = yaw0;
i = 1;
try
while i<50  % Simulate until convergence

uyaw(:,i) = mpc_yaw.get_u(yaw(:,i), yaw_position_reference); %Take the discretized matrices
